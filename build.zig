...
pub fn build(b: *std.Build) !void {
    ...
    // Module Setup
    const zig_objective_c_runtime_module = build_module.ZigObjectiveCRuntimeModule.init(b, &build_config);

    // Examples Setup
    const basic_example_exe = build_module.ExampleExe.init(b, &build_config, &zig_objective_c_runtime_module, "basic");

    // Test Setup
    const tests_exe = build_module.TestsExe.init(b, &build_config, &zig_objective_c_runtime_module);
